...
with open('../../../Data/SemEval2020/ulscd_eng/graded.txt', 'r') as infile:
    ...
    ranked_gold = [r[0] for r in sorted([(t, g) for t, g in zip(targets, graded_gold)], key=lambda x: x[1])]
...
for method, results in grouped:
    ...
    # Spearmans rho
    ranked_preds = [r[0] for r in sorted([(t, p) for t, p in zip(targets, preds)], key=lambda x: x[1])]
